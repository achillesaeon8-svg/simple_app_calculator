...
class CalculatorUI(FundamentalOperations):
    ...
    @staticmethod
    def draw_history_view(app, history_logs, controller):
        for widget in app.button_frame.winfo_children():
            widget.destroy()
        
        title = tkinter_library.Label(app.button_frame, text="Calculation History", 
                                     font=("Arial", 12, "bold"), bg="#34495e", fg="white")
        title.pack(pady=10)

        history_text = "\n".join(history_logs) if history_logs else "No history yet."
        log_label = tkinter_library.Label(app.button_frame, text=history_text, 
                                         font=("Arial", 10), bg="#34495e", fg="#ecf0f1", justify="left")
        log_label.pack(expand=True, fill="both", padx=10)

        back_btn = tkinter_library.Button(app.button_frame, text="Back to Calc", 
